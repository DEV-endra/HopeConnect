...
export default function Connect() {
    ...
    useEffect(() => {
        async function fun() {
            const token = localStorage.getItem("token");
            try {
                const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/users/chats?username=${encodeURIComponent(username)}`, {
                    method: "GET",
                    headers: {
    ...
    const newConversations = (peopleId) => {
        ...
        if (check === false) {
            async function fun() {
                const token = localStorage.getItem("token");
                try {
                    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/users/newconversation?peopleId=${encodeURIComponent(peopleId)}`, {
                        method: "GET",
                        headers: {
                            "Authorization": `Bearer ${token}`,
                            "Content-Type": "application/json"
